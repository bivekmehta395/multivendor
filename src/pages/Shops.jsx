...
const Shops = () => {
    ...
    return (
        <div>
            <Headers />
            <section className='bg-[url("http://localhost:3000/images/banner/2.jpg")] h-[220px] mt-6 bg-cover bg-no-repeat relative bg-left'>
                <div className='absolute left-0 top-0 w-full h-full bg-[#2422228a]'>
                    <div className='w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto'>
                        <div className='flex flex-col justify-center gap-1 items-center h-full w-full text-white'>
                            <h2 className='text-xl font-bold'>MultiVendor.com</h2>
                        ...
                        <div className={`w-3/12 md-lg:w-4/12 md:w-full pr-8 ${filter ? 'md:h-0 md:overflow-hidden md:mb-6' : 'md:h-auto md:overflow-auto md:mb-0'}`}>
                            ...
                            <div className='py-2'>
                                {categorys.map((c, i) => (
                                    <div className='flex justify-start items-center gap-2 py-1' key={i}>
                                        <input checked={category === c.name} onChange={(e) => queryCategoey(e, c.name)} type="checkbox" id={c.name} />
                            ...
                            <div className='py-2 flex flex-col gap-5'>
                                ...
                                <Range
                                    ...
                                    onChange={(values) => {
                                        setState({ values });
                                        // Dispatch the product query when the range changes
                                        dispatch(query_products({
                                            low: values[0],
                                            high: values[1],
                                            category,
                                ...
                                />
                                <div>
                                    <span className='text-red-500 font-bold text-lg'>Npr{Math.floor(state.values[0])} - Npr{Math.floor(state.values[1])}</span>
                                </div>
                            </div>
